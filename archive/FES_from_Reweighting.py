    ...
    while line[0] == "#!":
        ...
        if line[2] == "min_" + name_cv_x:
            ...
            if line[2] != "max_" + name_cv_x:
                sys.exit(
                    error
                    % ("min_%s was found, but not max_%s !" % (name_cv_x, name_cv_x))
                )
            if line[3] == "pi":
                grid_max_x = np.pi
            else:
        ...
        if dim2 and line[2] == "min_" + name_cv_y:
            ...
            if line[2] != "max_" + name_cv_y:
                sys.exit(
                    error
                    % ("min_%s was found, but not max_%s !" % (name_cv_y, name_cv_y))
                )
            if line[3] == "pi":
                grid_max_y = np.pi
            else:
...
def calcFESpoint(start, end, point_x, point_y=None):
    # point_x: value of the first CV
    # point_y: value of the second CV
    # start: index of the first sample to consider
    # end: index of the last sample to consider
